...
# sudo rm -rf ./bin
# sudo make build
# img="ecr.aws/arn:aws:ecr:us-west-1:020023120753:repository/codebuild-standard-7:latest"
...
echo "[" >> results.json
echo "{" >> results_averages.json
echo "20mb_3gb_1layer_parallelism" >> results_averages.json
for i in $(seq 1 1); do
set -e
echo "{" >> results.json
for j in $(seq 1 1); do	
  ...
  MEMORY=$(cat /sys/fs/cgroup/${CGROUP}/memory.peak)
  CPU=$(cat /sys/fs/cgroup/${CGROUP}/cpu.stat)
  echo "Parallel: ${ECR_PULL_PARALLEL},Time: ${TIME},Speed: ${SPEED},Memory: ${MEMORY}, PLSM: ${PLSM}"
  tot_Mem=$(( ${MEMORY} / 1048576 ))
  echo "\"run-$j\" : {
    \"Parallel layers\": ${ECR_PULL_PARALLEL},
    \"Pull Time\": ${TIME},
